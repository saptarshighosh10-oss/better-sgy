Create one horizontal sprite strip for the 2D game character `axell` in animation `idle-lineup`.
...
Output exactly 8 separate animation frames arranged left-to-right in one single row. Each frame must show the same character: original large orange alien character with a bulky rounded muscular body. Bright orange skin. Bald head. Most distinctive feature: one single large round eye centered in the face (cyclops). Wide friendly mouth. No normal nose. Big thick arms, large hands. Wears simple dark gray shorts or minimal outfit. Stands slightly wider than a human, thick legs, shorter proportions than the hero characters but much wider and bulkier..

Style contract: clean 2D cartoon game sprite style, vector-clean edges but raster output, bold outline, flat cel shading, readable full-body poses, no 3D render, no glossy app icon lighting. Additional user style notes: clean 2D comic flat colors, thick black outlines, orange and blue single eye, bold alien silhouette, no gradients, friendly expression despite massive size, animated series style.
Camera/view: front-facing.
Animation action: standing in a lineup for a family photo with the hero group, weight shifting slightly, single eye blinking slowly once during the loop, wide friendly smile, big arms at sides looking oddly cheerful about the whole thing.
