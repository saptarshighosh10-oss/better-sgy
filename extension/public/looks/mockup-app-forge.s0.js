...
function viewSettings(){
  ...
  const themes = THEMES.map(([k,col,title]) =>
    `<div class="theme ${state.theme===k?'on':''} ${dark?'dim':''}" data-theme="${k}" style="background:${col}" title="${dark?'switch to Light to use paper tints':title}"></div>`
  ).join('');
