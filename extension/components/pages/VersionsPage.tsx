...
function VersionViewer({ version, onBack }: { version: Version; onBack: () => void }) {
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', background: T.bg }}>
      <div style={{
        flexShrink: 0, display: 'flex', alignItems: 'center', gap: 12, padding: '10px 16px',
        ...
        <span style={{ fontSize: AT.caption, color: T.muted }}>{version.tagline}</span>
      </div>
      {failed ? (
...
export function VersionsPage() {
  ...
  if (open) {
    return <VersionViewer version={open} onBack={() => setOpen(null)} />;
  }
            ...
            >
              ...
              <div style={{ padding: '18px 20px 20px' }}>
                ...
                <span style={{ display: 'inline-flex', gap: 4, marginTop: 14, fontSize: AT.sub, color: T.primary, letterSpacing: AT.trackBody }}>
                  Open this version <span aria-hidden="true">›</span>
                </span>
              </div>
            </button>
          ))}
